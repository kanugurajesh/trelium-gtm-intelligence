...
| Evidence store + E1/E2 verifier | COMPLETE | `evidence/store.py`, `evidence/verify.py`, 19 tests; `scripts/verify_corpus.py` re-checks every cited quote in every brief, optionally against the committed git blobs | `append_index_record`/`load_index` still unused dead code (unchanged, harmless). Found and fixed: 11 committed snapshots had been rewritten CRLF→LF by git normalisation (`.gitattributes` now stores `evidence/raw/**` as bytes) | Run `verify_corpus.py --committed` before every commit that touches evidence |
